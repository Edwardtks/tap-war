...
// These keys will be pulled from your .env file in Vercel/Local
 ...
 * - WebSocket Broadcasting (1s Batching)
 * - Auto-Reconnect
    ...
    return (
      <div className={`fixed inset-0 flex flex-col items-center justify-center ${isRed ? 'bg-red-600' : 'bg-blue-600'} text-white overflow-hidden touch-none select-none`}>
        <div className="absolute top-8 text-center z-10 pointer-events-none">
          <div className="text-6xl font-black drop-shadow-xl font-mono">
        ...
        <button
          ...
          style={{
            ...
            transform: isChaos 
              ? 'translate(0, 0)'
              : 'translate(-50%, -50%) ' + (isPressed ? 'scale(0.95) translateY(10px)' : 'scale(1) translateY(0)'),
            boxShadow: isPressed ? '0 0 0 rgba(0,0,0,0.3)' : '0 10px 0 rgba(0,0,0,0.3)',
            transition: isChaos ? 'none' : 'transform 75ms'
          }}
        >
          <span className="text-8xl select-none pointer-events-none">
            {isRed ? '🔥' : '💧'}
          </span>
        </button>
      </div>
    ...
    return (
      <div className={`flex flex-col items-center justify-center min-h-screen p-6 transition-colors duration-500 ${isRed ? 'bg-red-950 text-red-100' : 'bg-blue-950 text-blue-100'}`}>
        <div className="text-center space-y-2 mb-12">
          ...
          <h1 className={`text-6xl font-black uppercase tracking-tighter ${isRed ? 'text-red-500' : 'text-blue-500'} drop-shadow-lg`}>
            TEAM {playerState.team}
          </h1>
        </div>
        <div className="w-full max-w-sm bg-black/30 backdrop-blur-sm border border-white/10 rounded-2xl p-8 text-center">
          <div className="w-20 h-20 mx-auto rounded-full bg-white/10 flex items-center justify-center text-3xl mb-4 border-2 border-white/20">
            {isRed ? '🔥' : '💧'}
          </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-black text-white p-6">
      <div className="mb-8 text-center">
        <h1 className="text-4xl font-black tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-blue-500 mb-2">
          TAP WAR
        </h1>
        ...
        <form onSubmit={handleJoin} className="flex flex-col space-y-4">
          <div>
            ...
            <input 
              ...
              className="w-full bg-zinc-950 border-2 border-zinc-800 text-white text-lg font-bold rounded-xl px-4 py-3 focus:outline-none focus:border-red-500 transition-colors placeholder:text-zinc-700"
            />
          </div>
          <button 
            type="submit"
            disabled={loading || !inputName}
            className="w-full bg-white text-black font-black text-xl py-4 rounded-xl hover:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed transition-all active:scale-95"
          >
...
const HostView = () => {
  ...
  useEffect(() => {
    ...
    const dbChannel = supabase
      .channel('host_db_sync')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'players' }, (payload) => {
        setPlayers(prev => [...prev, payload.new]);
      })
      .subscribe();

    // Subscribe to game clicks
      ...
      <header className="w-full max-w-6xl flex justify-between items-end mb-8 border-b border-zinc-800 pb-4">
        <div>
          ...
          <p className="text-zinc-500">
            Room Status: <span className={`font-mono font-bold ${gameState.status === 'PLAYING' ? 'text-green-500 animate-pulse' : 'text-yellow-500'}`}>{gameState.status}</span>
          </p>
        </div>
        <div className="text-right">
          <div className="text-sm text-zinc-400">Time Remaining</div>
          <div className={`text-5xl font-mono font-bold ${timeLeft <= 10 && gameState.status === 'PLAYING' ? 'text-red-500 animate-pulse' : 'text-white'}`}>
            {gameState.status === 'FINISHED' ? '0.0s' : timeLeft.toFixed(1) + 's'}
          </div>
        </div>
      </header>
      ...
      {gameState.status === 'FINISHED' && (
        <div className="w-full max-w-6xl mb-12 flex flex-col gap-8 animate-in zoom-in duration-500">
          <div className="py-12 bg-zinc-900/50 border border-zinc-700 rounded-3xl text-center">
            ...
            <h1 className={`text-9xl font-black uppercase tracking-tighter drop-shadow-[0_0_30px_rgba(255,255,255,0.2)] ${gameState.winner === 'RED' ? 'text-red-500' : gameState.winner === 'BLUE' ? 'text-blue-500' : 'text-gray-400'}`}>
              {gameState.winner === 'DRAW' ? 'DRAW!' : `TEAM ${gameState.winner}`}
            </h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            {sortedPlayers.length === 0 && (
              <div className="col-span-3 text-center text-zinc-500 py-8 italic">No clicks recorded!</div>
            )}
          </div>
        </div>
      )}
      ...
      {gameState.status === 'PLAYING' && (
        <div className="w-full max-w-6xl mb-12 animate-in fade-in zoom-in duration-500">
          ...
          <div className="relative h-24 w-full bg-zinc-900 rounded-2xl overflow-hidden border-4 border-zinc-800 shadow-[0_0_50px_rgba(0,0,0,0.5)]">
            ...
            <div 
              className="absolute top-0 bottom-0 left-0 bg-gradient-to-r from-red-900 to-red-600 transition-all duration-300 ease-out"
      ...
      {gameState.status !== 'FINISHED' && (
        <main className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-6xl flex-grow opacity-50 hover:opacity-100 transition-opacity">
          <div className="bg-zinc-900/30 border border-red-900/30 rounded-xl flex flex-col relative overflow-hidden h-[40vh]">
            ...
            <div className="p-4 bg-zinc-900/80 border-b border-red-900/20 flex justify-between items-center backdrop-blur-sm">
              <h2 className="text-xl font-black text-red-500 uppercase italic">Team Red</h2>
              <span className="bg-red-900/20 text-red-500 px-3 py-1 rounded text-sm font-mono">{redPlayers.length} Joined</span>
            </div>
            <div className="flex-grow overflow-y-auto p-4 space-y-2 scrollbar-hide">
              <div className="grid grid-cols-2 gap-2">
                  {redPlayers.map(p => (
                    <div key={p.id} className="bg-red-950/40 border border-red-900/20 rounded px-3 py-2 text-red-200 font-mono text-xs truncate">{p.nickname}</div>
                  ))}
                </div>
            </div>
          </div>
          ...
          <div className="bg-zinc-900/30 border border-blue-900/30 rounded-xl flex flex-col relative overflow-hidden h-[40vh]">
            ...
            <div className="p-4 bg-zinc-900/80 border-b border-blue-900/20 flex justify-between items-center backdrop-blur-sm">
              <h2 className="text-xl font-black text-blue-500 uppercase italic">Team Blue</h2>
              <span className="bg-blue-900/20 text-blue-500 px-3 py-1 rounded text-sm font-mono">{bluePlayers.length} Joined</span>
            </div>
            <div className="flex-grow overflow-y-auto p-4 space-y-2 scrollbar-hide">
                <div className="grid grid-cols-2 gap-2">
                  {bluePlayers.map(p => (
                    <div key={p.id} className="bg-blue-950/40 border border-blue-900/20 rounded px-3 py-2 text-blue-200 font-mono text-xs truncate">{p.nickname}</div>
                  ))}
                </div>
            </div>
          </div>
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans selection:bg-red-500 selection:text-white overflow-hidden">
      ...
      <div className="fixed bottom-4 right-4 z-50">
        <button onClick={() => setViewMode(prev => prev === 'HOST' ? 'PLAYER' : 'HOST')} className="text-xs font-bold font-mono bg-zinc-800 hover:bg-zinc-700 text-white px-4 py-2 rounded-full border border-zinc-600 transition-colors shadow-[0_0_15px_rgba(0,0,0,0.5)] hover:scale-105">
          {viewMode === 'HOST' ? 'Switch to Player View' : 'Switch to Host View'}
        </button>
      </div>
    </div>
  );
